  ...
  return (
    <div className="card">
      ...
      <form onSubmit={crearMesa} className="row" style={{ gap: 8, marginBottom: 12 }}>
        <input placeholder={t("mesas.namePlaceholder")} value={nombreMesa} onChange={(e) => setNombreMesa(e.target.value)} style={{ flex: 1 }} />
        <button className="ghost" type="submit">{t("mesas.addTable")}</button>
      </form>

      {mesas.length === 0 ? (
        <p className="muted small">{t("mesas.createFirst")}</p>
      ) : (
                ...
                {comanda ? (
                  <>
                    <div className="row spread" style={{ marginTop: 6 }}>
                      <span className="muted small">{comanda.lineas.length} {t("mesas.items")}</span>
                      <strong>{money(comanda.total)}</strong>
                    </div>
                    <button className="primary small" style={{ width: "100%", marginTop: 8 }} onClick={() => setActiva(comanda.id)}>{t("mesas.openOrder")}</button>
                  </>
                ) : (
                  <button className="ghost small" style={{ width: "100%", marginTop: 8 }} onClick={() => abrirComanda(m)}>{t("mesas.openOrder")}</button>
                )}
              </div>
            );
          })}
...
function ComandaEditor({ comandaId, onCerrar }: { comandaId: string; onCerrar: () => void }) {
  ...
  return (
    <div className="card pop" style={{ background: "var(--surface-2)", marginTop: 12, borderColor: "var(--brand-600)" }}>
      <div className="row spread"><h3 style={{ margin: 0 }}>{t("mesas.order")}</h3><button className="ghost small" onClick={onCerrar}>{t("mesas.close")}</button></div>
      ...
      <form onSubmit={agregar} className="grid grid-2" style={{ marginTop: 8 }}>
        <div><label>{t("mesas.dish")}</label><input value={l.nombre} onChange={(e) => setL({ ...l, nombre: e.target.value })} required /></div>
        <div><label>{t("mesas.price")}</label><input type="number" step="0.01" min="0" value={l.precioUnit} onChange={(e) => setL({ ...l, precioUnit: e.target.value })} required /></div>
        <div><label>{t("mesas.quantity")}</label><input type="number" min="1" value={l.cantidad} onChange={(e) => setL({ ...l, cantidad: e.target.value })} /></div>
        <div><label>{t("mesas.notesOpt")}</label><input value={l.notas} onChange={(e) => setL({ ...l, notas: e.target.value })} placeholder={t("mesas.notesPh")} /></div>
        <button className="ghost" type="submit" style={{ gridColumn: "1 / -1" }}>{t("mesas.addToOrder")}</button>
      </form>
      {error && <p className="error small">{error}</p>}
      <div className="row spread" style={{ marginTop: 10, fontSize: 18, fontWeight: 800 }}><span>{t("mesas.total")}</span><span className="grad-text">{money(comanda.total)}</span></div>
      <button className="primary" style={{ width: "100%", marginTop: 10 }} onClick={cobrar} disabled={comanda.lineas.length === 0}>{t("mesas.chargeAndFree")}</button>
    </div>
  );
}
